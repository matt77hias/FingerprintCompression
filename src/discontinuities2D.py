...
def plot_large_coeffs_matrix(f, mode=pywt.MODES.ppd, level=4, threshold=0.1):
...
if __name__ == "__main__":
    plot_large_coeffs_matrix(matrix)
    plot_large_coeffs_matrix(matrix2)
    plot_large_coeffs_matrix(matrix3)
    
    S = matrix()
    #S = matrix2()
    #S = matrix3()
    utils.draw_coeffs2(S)
    #pylab.spy(S)
    task_fwt2(S)
    task_fft2(S)